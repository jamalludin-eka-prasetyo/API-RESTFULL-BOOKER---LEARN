...
class myCustomKeywords {
	...
	@Keyword
	def getTokenLogin(String username, String password) {
		KeywordUtil.logInfo("================ Keyword: Mengambil Token Login ================")
		
		def reqToken = ObjectRepository.findTestObject("Object Repository/Restful Booker/Create Token")
		reqToken.setVariables(
			[ ('username') : username, 
			  ('password') : password
			])
		ResponseObject response = WS.sendRequest(reqToken)
		
		if (response.getStatusCode() == 200) {
			String token = WS.getElementPropertyValue(response, 'token')
			KeywordUtil.logInfo("Keyword: Login Berhasil! Mendapatkan token: " + token)
			GlobalVariable.token = token
			return token	
		} else {
			KeywordUtil.markWarning("Keyword: Login Gagal. Status Code: " + response.getStatusCode() + ", Body: " + response.getResponseText())
			return null
		}
